...
from typing import Optional, Callable  # 类型注解，Optional[X] 表示 X 或 None
...
# 第3步：采集全部250条数据
# ============================================================
def scrape_top250(delay: float = 1.0, dry_run: bool = False, progress_callback: Optional[Callable[[int, int, int], None]] = None) -> list[dict]:  # delay=请求间隔秒数, dry_run=是否只采集第一页
    """
    主采集逻辑：
    ...
    for i, url in enumerate(tqdm(urls, desc="  采集进度", unit="页"), start=1):  # 带进度条遍历URL列表，i从1开始
        ...
        if progress_callback:
            progress_callback(i, len(urls), len(all_movies))
